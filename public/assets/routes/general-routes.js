...
module.exports = function(app) {
	...
	const scrapeSite = function(callback) {
		...
		request("https://www.smashingmagazine.com/articles/", function(
			...
			$(".article--post").each(function(i, element) {
				...
				// cheerio scrapes the html to get author, url and date of article
				var author = $(element)
				...
				if (headline && urlLink) {
					// save results in object and add to the results array
					results.push({
						headline: headline,
			...
			// callback function with the result data returned from the scrape
	...
	app.get("/scrape", function(req, res) {
		...
		scrapeSite(function(err, articleList) {
			...
			asyncForEach(articleList, async (item, index) => {
				console.log("Finding index[" + index + "]: " + item.headline);

				db.Article.find({ headline: item.headline.trim() })
					.then(function(dbResult) {
						console.log("Mongo has: ");
						console.log(dbResult);

						if (dbResult.length === 0) {
							// if don't find it, we'll need to add it
							console.log("add item to array to add to mongo.");
							console.log(item);
